...
export async function listAds(
  ...
  if (!db) return { ads: [], total: 0, page, limit };
...
export async function listAdvertisers(
  ...
  if (!db) return { advertisers: [], total: 0, page, limit };
...
export async function getStats(db: D1Database) {
  ...
  return {
    ...
    topAdvertisers: topAdvertisers?.results ? topAdvertisers.results.map(mapAdvertiser) : [],
  };
}
