...
import { aboutPageSchema, breadcrumbSchema, faqSchema, howToSchema, webPageSchema } from "../lib/schema";
...
export default function AboutPage() {
  ...
  return (
    <>
      <Seo
        title="About the ChatGPT Ads Library — How It Works & FAQs"
        description="Learn how the ChatGPT Ads Library works, what ad data it tracks, and how to search and filter ads running across ChatGPT. Includes FAQs and a step-by-step guide."
        path="/about"
        keywords="ChatGPT Ads Library about, how ChatGPT ads work, ChatGPT advertising FAQ, search ChatGPT ads, OpenAI advertising transparency"
        jsonLd={[
          breadcrumbSchema([
            { name: "Home", path: "/" },
        ...
        <ol className="mt-4 space-y-3 text-zinc-600 dark:text-zinc-300">
          {HOW_TO_STEPS.map((step, i) => (
            <li key={step.name} className="flex gap-3">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-500 text-xs font-bold text-white" aria-hidden="true">
                {i + 1}
              </span>
              <div>
